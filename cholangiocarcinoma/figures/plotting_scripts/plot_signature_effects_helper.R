plot_cca_signature_effects = function(mutational_effects = NULL,
  ...
  # Calculate average effect share per group
  ...
  # Identify aging-related labels
  aging_labels = c(
    names(signature_groupings)[sapply(signature_groupings, function(x) identical(x, "SBS1"))],
    names(signature_groupings)[sapply(signature_groupings, function(x) identical(x, "SBS5"))]
  )
  
  # Remove SBS1/SBS5 from main list and append them at the end
  non_aging = setdiff(mean_share_order, aging_labels)
  final_order = c(other_label, aging_labels, non_aging)
